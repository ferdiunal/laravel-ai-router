...
- Uyumlu `/models` endpoint sunmayan gateway'ler için opsiyonel declared model listesi.
- Live `/models` discovery modu (`enabled` veya `disabled`).
- Credential validation method: `GET /models` için `models`, minimal `POST /chat/completions` isteği için `chat` ve validation model.

Persistence ve request dispatch öncesinde güvenlik kısıtları uygulanır:

- Base URL public `https://` olmak zorundadır.
- Base URL API root olmalıdır; örneğin `https://api.example.com/v1`. Final `/chat/completions`, `/completions` veya `/models` endpoint'i kabul edilmez.
...
Uyumlu `/models` endpoint sunmayan OpenAI-compatible gateway'ler için live model discovery'yi kapatıp route edilebilir chat modellerini açıkça declare et. Bu alanları provider definition eklerken verebilir veya sonradan güncelleyebilirsin:
...
Declared model satırları provider key refresh sırasında `definition` source ile cachelenir. Exact model routing declared modelleri kullanabilir. `auto` / `random_provider` için declared model JSON metadata içinde `auto_enabled: true` ver veya key refresh sonrası `php artisan laravel-ai-router:provider:models` ile cached satırları seç; basit comma-separated ID'ler default olarak exact-routeable kalır ama auto için seçilmez.
...
Live model discovery routable provider'lar için geneldir: `/models` içinden gelen geçerli satırlar ID değeri `:free` ile bitmese bile cachelenir. Free-tier bilgisi metadata olarak saklanır (`is_free`); NVIDIA NIM live satırları free credit-backed model olarak işaretlenir (`free` + `credits-based`), diğer non-free live satırlar varsayılan olarak `credits-based` budget label alır. Exact live model ID'ler cache row `auto` için seçili olmasa bile doğrudan route edilebilir. Automatic routing, cache görünürlüğünden daha dardır: yeni refresh edilen satırlar yalnızca provider adapter'ın chat kontratıyla ve paketin provider-specific safe policy'siyle eşleştiğinde `auto` için seçilir. Google AI Studio auto seçimi stable Gemini `generateContent` modelleriyle sınırlıdır (`gemini-2.5-flash`, `gemini-2.5-flash-lite`); embedding, media, live/Interactions ve live probe edilmemiş preview/pro modeller görünürlük için cache'te kalır ama auto'ya seçilmez. Cloudflare Workers AI auto seçimi, ek modeller live probe ile doğrulanana kadar conservative Workers AI chat-completions baseline ile sınırlıdır. Key-backed provider'ları production'da kullanmadan önce upstream quota ve billing şartlarını ayrıca gözden geçir.
